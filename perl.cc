...
/* ---- chmod over a host import --------------------------------------------
 * wasi-libc's chmod is a stub (WASI preview1 cannot change file modes),
 * which turns build systems' permission fixups (Module::Build makes
 * blib trees writable before rebuilding them) into hard failures. The
 * runtime provides a path_chmod import; this definition shadows the
 * libc stub at link. Paths are absolutized against the guest cwd and
 * passed preopen-relative, matching path_open's convention. */
__attribute__((import_module("wasi_snapshot_preview1"), import_name("path_chmod")))
extern "C" int __goperl_host_path_chmod(const char *path, int len, unsigned int mode);

/* Absolutize against the guest cwd and strip the leading '/' (paths cross
 * the imports preopen-relative, like path_open's). Returns p or buf. */
static const char *goperl_import_path(const char *path, char *buf, size_t bufsz) {
    const char *p = path;
    if (p[0] != '/') {
        char cwd[3584];
...
/* chdir with lexical normalization: wasi-libc keeps the handed-over path
 * verbatim (".." included) as its cwd, which breaks cwd-relative
 * operations once a traversed component is deleted (File::Path's rmtree
 * chdirs down, works, chdirs back up via "..", and then deletes what it
 * just left). Callers reach this through the shim header's chdir macro;
 * this file sees the real libc chdir. */
extern "C" int __goperl_chdir_normalized(const char *path) {
    if (path == NULL) {
        errno = EFAULT;
        return -1;
    }
...
extern "C" int chmod(const char *path, mode_t mode) {
    ...
    char buf[4096];
    const char *p = goperl_import_path(path, buf, sizeof(buf));
    int rc = __goperl_host_path_chmod(p, (int)strlen(p), (unsigned int)mode);
    if (rc != 0) {
        errno = (rc < 0) ? -rc : rc;
        return -1;
    }
...
/* ---- stat/lstat with real permission bits --------------------------------
 * WASI's filestat carries no permission bits, so wasi-libc's stat family
 * reports st_mode with the file type only. Tools that round-trip modes —
 * Module::Build's make_executable does chmod((stat $f)[2] | 0111) —
 * would then strip every permission. These overrides run the libc stat
 * (via fstatat, which stays visible) and merge the real bits from the
 * path_filestat_mode import. */
__attribute__((import_module("wasi_snapshot_preview1"), import_name("path_filestat_mode")))
extern "C" int __goperl_host_path_filestat_mode(const char *path, int len,
                                                int follow, unsigned int *mode_out);

static int goperl_stat_common(const char *path, struct stat *st, int follow) {
    int rc = fstatat(AT_FDCWD, path, st, follow ? 0 : AT_SYMLINK_NOFOLLOW);
    if (rc != 0) return rc;
    char buf[4096];
    const char *p = goperl_import_path(path, buf, sizeof(buf));
    unsigned int mode = 0;
    if (__goperl_host_path_filestat_mode(p, (int)strlen(p), follow, &mode) == 0)
        st->st_mode = (st->st_mode & S_IFMT) | (mode_t)(mode & 07777);
    return 0;
}

extern "C" int stat(const char *path, struct stat *st) {
    return goperl_stat_common(path, st, 1);
}

extern "C" int lstat(const char *path, struct stat *st) {
    return goperl_stat_common(path, st, 0);
}
